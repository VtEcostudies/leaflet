...
function addBoundaries() {
    var stateLayer = omnivore.kml('../kml/VT_Data__State_Boundary.kml');
    var countyLayer = omnivore.kml('../kml/VT_Data__County_Boundaries.kml');
    var townLayer = omnivore.kml('../kml/VT_Data__Town_Boundaries.kml');
    var bioPhysicalLayer = omnivore.kml('../kml/VTBiophysicalregions.kml').addTo(valMap);
    ...
    //if (!grpFeature) {grpFeature = L.featureGroup(bioPhysicalLayer);}
    
    //grpFeature.bringToBack();
}
...
if (document.getElementById("valLoadOnOpen")) {
    window.addEventListener("load", function() {
        ...
        valMap.options.maxZoom = 12;
        //initValOccCanvas();
        //addValOccCanvas('Bombus borealis');
        addBoundaries();
        getSpeciesListData();
        if (grpFeature) grpFeature.bringToFront();
...
function getSpeciesListData() {
    ...
    Object.keys(speciesList).forEach(function(key) {
        cmGroup[key] = L.layerGroup().addTo(valMap); //create a new, empty, single-species layerGroup to be populated from API
        grpFeature.addLayer(cmGroup[key]);
        cgColor[key] = colors[i++];
        cmCount[key] = 1;
        console.log(`Add species group ${key} with color ${cgColor[key]}`);
        addValOccCanvas(key);
        speciesLayers.addOverlay(cmGroup[key], key);
    });
}